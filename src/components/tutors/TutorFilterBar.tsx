...
export default function TutorFilterBar({ filters, onFilterChange, onResetFilters, totalResults }: TutorFilterBarProps) {
  const hasActiveFilters = filters.subject !== '' || 
    filters.minPrice !== '' || 
    filters.maxPrice !== '' || 
    filters.language !== '' || 
    filters.country !== '' || 
    ...
    <div className="w-full space-y-4 mb-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
        <div className="relative group">
          <label className="text-[10px] font-black uppercase tracking-widest text-navy-300 dark:text-cream-400/40 absolute left-10 top-2 z-10">I want to learn</label>
        ...
        <div className="relative">
          <label className="text-[10px] font-black uppercase tracking-widest text-navy-300 dark:text-cream-400/40 absolute left-4 top-2 z-10">Price per lesson</label>
          <select 
        ...
        <div className="relative">
          <label className="text-[10px] font-black uppercase tracking-widest text-navy-300 dark:text-cream-400/40 absolute left-4 top-2 z-10">Country of birth</label>
          <select 
        ...
        <div className="relative">
          <label className="text-[10px] font-black uppercase tracking-widest text-navy-300 dark:text-cream-400/40 absolute left-4 top-2 z-10">I'm available</label>
          <select 
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="flex-1 flex flex-wrap items-center gap-2">
            {[
                ...
                { id: 'language', label: 'Also speaks' },
                { id: 'nativeSpeaker', label: 'Native speaker', type: 'toggle' },
                { id: 'category', label: 'Tutor categories' },
            ].map(f => (
                f.type === 'toggle' ? (
                    <button 
                        key={f.id}
                        onClick={() => onFilterChange(f.id, !filters.nativeSpeaker)}
      ...
      <div className="pt-4">
          <h2 className="text-2xl font-display font-black text-navy-600 dark:text-cream-200">
            <span className="text-gold-500">{totalResults.toLocaleString()}</span> {filters.subject ? SUBJECT_LABELS[filters.subject as Subject] : 'Tutors'} available
          </h2>
      </div>
    </div>
  );
