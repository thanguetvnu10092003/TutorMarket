...
export default function PricingManager() {
  ...
  // Exchange rates placeholder (in real app, fetch from API)
  const rates: any = {
    'VND': 25000,
    'USD': 1,
    'SGD': 1.34,
    ...
    try {
      ...
      if (res.ok) {
        ...
        DURATIONS.forEach(d => {
            ...
            pMap[d] = {
                price: found?.price || (d === 60 ? 250000 : 0),
                isEnabled: found?.isEnabled || (d === 60)
            };
        });
        setPrices(pMap);
        setCurrency(json.data[0]?.currency || 'VND');
        setLastUpdated(json.data[0]?.updatedAt);
      }
    } catch (error) {
    ...
    <div className="glass-card p-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="space-y-4">
          {DURATIONS.map(dur => (
            <div key={dur} className={`p-4 rounded-2xl border transition-all ${prices[dur].isEnabled ? 'border-gold-400/40 bg-gold-400/5' : 'border-navy-100 dark:border-navy-400/10 opacity-60'}`}>
              <div className="flex items-center justify-between mb-3">
                ...
                >
                  <span className={`absolute top-0.5 w-3 h-3 rounded-full bg-white transition-transform ${prices[dur].isEnabled ? 'translate-x-4 left-0.5' : 'left-0.5'}`} />
                </button>
              </div>
              ...
              <div className="flex items-center gap-3">
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    onChange={e => setPrices(p => ({ ...p, [dur]: { ...p[dur], price: parseFloat(e.target.value) || 0 } }))}
           ...
           <div className="p-6 rounded-3xl bg-navy-600 dark:bg-navy-700 text-white relative overflow-hidden shadow-xl">
             ...
             <ul className="space-y-4 relative z-10">
               ...
               <li className="flex gap-3">
                  ...
                  <p className="text-xs text-cream-200/80 leading-relaxed">Students in different regions will see your rates converted to their local currency (approximate).</p>
               </li>
               {lastUpdated && (
